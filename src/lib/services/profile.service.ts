 ...
 * @example
 * ```typescript
 * const profile = await findProfileByUserId(supabase, "user-uuid");
 * if (!profile) {
 *   console.log("Profile not found");
 * }
 * ```
 */
export async function findProfileByUserId(supabase: SupabaseClient, userId: string) {
 ...
 * ```
 */
export async function updateProfile(supabase: SupabaseClient, userId: string, data: UpdateProfileCommand) {
